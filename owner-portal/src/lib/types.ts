...
/**
 * A staff-curated highlight from the physical kiosk's own product catalog.
 * Deliberately thin — just the sku plus an optional note — since name/
 * price/image already live on the kiosk (Product.kt) and would only drift
 * out of sync if duplicated here. See KioskProduct below for what the
 * owner-portal itself knows about that catalog, for the picker UI.
 */
export type StaffPickDocument = {
  sku: string;
  note?: string;
  createdAt: Timestamp;
};

export type StaffPick = StaffPickDocument & { id: string };

/** A row from kioskCatalog.json — a static snapshot of the kiosk's real
 * Product.kt, regenerated by scripts/generateKioskCatalog.mjs. */
